...
class KiethleyFG(FG.FG):
    ...
    def uploadWaveform(self, data, scl=True, name="VOLATILE"):
        ...
        st="DATA:DAC {}, {}".format(name,datStr)
        print(st[:40])
        self.handle.write(st)
    ...
    def setLowHigh(self, low, high, chanNum=0):
        if chanNum>0:
            raise ValueError("only one channel")
    ...
    def setAmp(self, val, chanNum=0):
        self.handle.write("VOLT {}".format(val))
